...
def input_fn(filenames, batch_size=32, num_epochs=1, perform_shuffle=False):
    ...
    def decode_libsvm(line):
        ...
        labels = tf.string_to_number(columns.values[-1], out_type=tf.float32)
...
def main(_):
    ...
    # ------init Envs------
    tr_files = glob.glob("%s/train.data" % FLAGS.data_dir)
    random.shuffle(tr_files)
    print("tr_files:", tr_files)
    va_files = glob.glob("%s/test.data" % FLAGS.data_dir)
    print("va_files:", va_files)
    te_files = glob.glob("%s/test.data" % FLAGS.data_dir)
